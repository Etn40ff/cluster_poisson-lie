...
\section{$\sl_{n+1}$ Case}
\sayD{ToDo: Translate this into Weyl group notation using a reduced word for $w_0$.}

Let $\alpha_1,\ldots,\alpha_n$ be the simple roots.
The positive roots of $\sl_n$ are given by $\Phi^+=\{\alpha_i+\alpha_{i+1}+\cdots+\alpha_j:1\le i\le j\le n\}$, in what follows we abbreviate $\alpha_{ij}=\alpha_i+\alpha_{i+1}+\cdots+\alpha_j$.

The positive (resp. negative) part $\sl_{n+1}^+$ (resp. $\sl_{n+1}^-$) is generated by $\xi_i$ (resp. $\xi_{-i}$) for $1\le i\le n$.
The Cartan $\sl_{n+1}^0$ is spanned by $\xi_{i^\vee}$ for $1\le i\le n$.
Define elements $\xi_{ij}\in\sl_{n+1}$ for $1\le i\le j\le n$ recursively by
\[\xi_{ii}=\xi_i,\qquad \xi_{i,j+1}=[\xi_i,\xi_{i+1,j+1}],\]
where $[\cdot,\cdot]:\sl_{n+1}\times\sl_{n+1}\to\sl_{n+1}$ is the standard Lie bracket.
Analogously, define the elements $\xi_{-ij}\in\sl_{n+1}$ for $1\le i\le j\le n$ recursively by
\[\xi_{-ii}=\xi_{-i},\qquad \xi_{-i,j+1}=[\xi_{-i+1,j+1},\xi_{-i}].\]
\sayD{Note the change in order here, this eliminates some ugly signs below.}
\sayD{This notation sucks but it should be clear what I mean.}
\begin{lemma}
  The elements $\xi_{ij}$ (resp. $\xi_{-ij}$) form a basis for $\sl_{n+1}^+$ (resp. for $\sl_{n+1}^-$).
  For any $i\le j$ and $i'\le j'$, we have 
  \[
    [\xi_{ij},\xi_{i'j'}]=
    \begin{cases}
      \xi_{ij'} & \text{if $i'=j+1$;}\\
      -\xi_{i'j} & \text{if $i=j'+1$;}\\
      0 & \text{otherwise.}
    \end{cases}
  \]
  \[
  ...
  \[
    [\xi_{k^\vee},\xi_{i'j'}]=
    \begin{cases}
      2\xi_{i'j'} & \text{if $i'=j'=k$;}\\
      \xi_{i'j'} & \text{if $i'=k<j'$ or $i'<k=j'$;}\\
      -\xi_{i'j'} & \text{if $i'=k+1$ or $j'=k-1$;}\\
      0 & \text{otherwise.}
    \end{cases}
  \]
  \[
  ...
  \[
    [\xi_{ij},\xi_{-i',j'}]=
    \begin{cases}
      -\xi_{-j+1,j'} & \text{if $i=i'$ and $j<j'$;}\\
      -\xi_{j'+1,j} & \text{if $i=i'$ and $j>j'$;}\\
      \xi_{i,i'-1} & \text{if $i<i'$ and $j=j'$;}\\
      \xi_{-i',i-1} & \text{if $i>i'$ and $j=j'$;}\\
      \sum\limits_{k=i}^j \xi_{k^\vee} & \text{if $i=i'$ and $j=j'$;}\\
      0 & \text{otherwise.}
    \end{cases}
  \]
\end{lemma}
