...
\usepackage{xparse}
\NewDocumentCommand{\minor}{ O{} O{#1} }{\Delta_{#1, #2}}
%\NewDocumentCommand{\minor}{ O{} O{#1} }{X_{#1, #2}}
\NewDocumentCommand{\function}{ O{} O{#1} O{\gamma} }{\Sigma_{#1, #2}^{#3}}
\NewDocumentCommand{\monomial}{ O{} O{#1} O{\gamma} }{M_{#1, #2}^{#3}}
...
\begin{definition}
  The generalized minor $\minor[\lambda][\mu]$ is the regular function whose value on $x\in u\mcG_0v^{-1}$ is $[u^{-1}xv]_0^{\omega_i}$.
\end{definition}
We are using here the notation of \cite{BFZ05}; previously this was $\Delta_\lambda^\mu$.
Alternatively, in the language of \cite{RSW17}, the minor $\minor[\lambda][\mu]$ is defined by the formula $\pi_\lambda\big(x v_\mu\big)/v_\lambda$.
...
\begin{remark} 
  ...
  With this setup the minor $\minor[\lambda][\mu]$ is the minor with rows $\{\lambda\}$ and columns $\{\mu\}$.
...
\begin{lemma}
  ...
  \[
    \frac{\{\minor[u\omega_i][vv'\omega_i],\minor[uu'\omega_j][v\omega_j]\}_{0,s_\gamma}}{\minor[u\omega_i][vv'\omega_i]\minor[uu'\omega_j][v\omega_j]}
    =
...
\begin{remark}
  For any generalized minor $\minor[\lambda][\mu]$ defined in terms of a minuscule representation (in particular all the one we care about for $SL_{n+1}$) and for any simple root $\alpha$,
  \[
    \xi^R_{\alpha} \minor[\lambda][\mu] = \minor[\lambda-\alpha][\mu]
    \qquad \mbox{and} \qquad
    \xi^L_{\alpha} \minor[\lambda][\mu] = \minor[\lambda][\mu+\alpha]
  \]
...
\begin{definition}
  ...
  A generalized minor $\minor[\lambda][\mu]$ is \newword{$\gamma$-tuned} if $\alpha^\vee(\mu)\leq0$ for all~$\alpha\in\Phi^+_1$ and $\beta^\vee(\lambda)\leq0$ for all $\beta\in\Phi^+_2$.
\end{definition}
By \cref{cor: tuned} if $\minor[\lambda][\mu]$ is a $\gamma$-tuned minor then it belongs to $\mcO^\gamma_-$.
Formally the constant function is also a $\gamma$-tuned minor, indeed $1=\minor[0]$.

\begin{example}
  Write $\lambda=\sum\lambda_i\omega_i$ and $\mu=\sum\mu_i\omega_i$.
  Then $\minor[\lambda][\mu]$ is $\gamma$-tuned if $\lambda_2\leq0$, $\lambda_3\leq0$, $\mu_1\leq0$, and $\mu_2\leq0$.
\end{example}
...
\begin{example}
  In our running example the minor $\minor[-\omega_3][-\omega_1+\omega_2]$ is $\gamma$-tunable but not $\gamma$-tuned. 
\end{example}

\begin{definition}
  Given a $\gamma$-tunable generalized minor $\minor[\lambda][\mu]$ define the \newword{$\gamma$-tuning function} $\varphi_\gamma$ by
  \[
    \varphi_\gamma(\minor[\lambda][\mu]) :=
    \begin{cases}
      1 & \mbox{if } \minor[\lambda][\mu] \mbox{ is $\gamma$-tuned}\\
      \minor[\omega_{i^*}][-\omega_i] & \mbox{if } \alpha^\vee_j(\mu) > 0 \mbox{ and } \gamma(\alpha_j) = \alpha_i\\
      \minor[-\omega_i][\omega_{i^*}] & \mbox{if } \alpha^\vee_j(\lambda) > 0 \mbox{ and } \gamma(\alpha_i) = \alpha_j\\
    \end{cases}
  \]
...
\end{definition}
\begin{example}
  Since $\alpha_2^\vee(-\omega_1+\omega_2) = 1$ and $\gamma(\alpha_2)=\alpha_3$, we have $\varphi_\gamma(\minor[-\omega_3][-\omega_1+\omega_2]) = \minor[\omega_1][-\omega_3]$.
\end{example}

\begin{remark}
  Note that the non-constant minors produced by $\varphi_\gamma$ are frozen cluster variables in the standard cluster structure.
\end{remark}

\begin{lemma}
  If $\gamma$ is \newword{aperiodic} in the sense of \cite{GSV19} then $\varphi_\gamma^k(\minor[\lambda][\mu])=1$ for any $\gamma$-tunable generalized minor and a sufficiently large $k$.
\end{lemma}
\begin{proof}
...
\begin{definition} 
  ...
  \[
    \monomial[\lambda][\mu] := \prod_{k=0}^\infty \varphi_\gamma^k(\minor[\lambda][\mu]).
  \]
  Since $\gamma$ is aperiodic this is a finite product.
\end{definition}

\begin{example}
  The minor $\minor[\omega_1-\omega_3][-\omega_1+\omega_3]$ is $\gamma$-tuned so $monomial[\omega_1-\omega_3][-\omega_1+\omega_3]=\minor[\omega_1-\omega_3][-\omega_1+\omega_3]$. 

  Since $\minor[-\omega_3][-\omega_1+\omega_2]$ is $\gamma$-tunable while $\varphi_\gamma(\minor[-\omega_3][-\omega_1+\omega_2])=\minor[\omega_1][-\omega_3]$ is $\gamma$-tuned, we have $\monomial[{-\omega_3}][{-\omega_1+\omega_2}] = \minor[-\omega_3][-\omega_1+\omega_2] \minor[\omega_1][-\omega_3]$.
\end{example}
...
\begin{proof}[Proof of \cref{prop: max compatible}]
  ...
  From the definition of $\mcL(\bfi)$ we get that any minor $\minor[\lambda][\mu]\in\mcS(\bfi,0)$ is such that 
  \[
...
\begin{proposition}
  ...
  Then any generalized minor in $\mcS(\bfi,0)$ is $\gamma$-tunable.
\end{proposition}
...
\[
  ...
  :=
  \Big\{
    \function[\lambda][\mu] \,\Big|\, (\lambda,\mu) \in \mcL(\bfi)
  \Big\}.
\]
Clearly, when $\gamma=0$ this new definition coincides with the one given above for $\mcS(\bfi,0)$.
...
\end{corollary}
\begin{proof}
  \Cref{prop: max tuned seed} guarantees that all the element in $\mcS(\bfi,\gamma)$ are defined.
  \Cref{conj: bracket comparison} says that the statement is equivalent to the fact that 
  \[
    \mcM(\bfi,\gamma)
    :=
    \Big\{
      \monomial[\lambda][\mu] \,\Big|\, (\lambda,\mu) \in \mcL(\bfi)
    \Big\}.
  \]
  is a log canonical set with respect to the bracket $\{\cdot,\cdot\}_{0,s_\gamma}$.
  This, in turn, follows immediately using the Leibniz rule and the fact that elements in $\mcM(\bfi,\gamma)$ are monomials in the minors from $\mcS(\bfi,0)$.
\end{proof}

\begin{remark}
  The set $\mcM(\bfi,\gamma)$ consists of algebraically independent funtions. 
  Indeed the set $\mcS(\bfi,0)$ has the same property and the change from one to the other is triangular.
\end{remark}
...
\[
  \mcF(\bfi)
  :=
  \Big\{\minor[\omega_j][v^{-1}\omega_j] \,\Big|\, j \in [1,n] \Big\}
  \cup
  \Big\{\minor[u\omega_j][\omega_j] \,\Big|\, j \in [1,n] \Big\}.
\]
